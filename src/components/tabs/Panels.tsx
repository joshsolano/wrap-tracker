...
export default function Panels() {
  ...
  const sortedProjects = useMemo(() => {
    ...
    for (const r of logs) {
      if (r.project_id) {
        const t = r.finish_ts ? new Date(r.finish_ts).getTime() : 0
        if (!lastActivity.has(r.project_id) || t > lastActivity.get(r.project_id)!) {
          lastActivity.set(r.project_id, t)
        }
      }
    }
  ...
  function isProjComplete(projId: string): boolean {
    ...
    return (proj.panels ?? []).every(pnl =>
      logs.some(r => r.panel_id === pnl.id && r.project_id === projId && r.status === 'Complete')
    )
  }
  ...
  async function handleCreateProject() {
    ...
    const { error } = await createProject({
      name: libNewProjectName.trim(),
      projectType: libProjType,
      dueDate: libDueDate || undefined,
    })
    setSaving(false)
    if (error) {
  ...
  async function handleAddPanel() {
    ...
    if (libH && !parseDim(libH)) {
      setLibHErr('Use inches (48) or feet (4ft)')
      return
    }
    if (libW && !parseDim(libW)) {
      setLibWErr('Use inches (72) or feet (6ft)')
      return
    }
    setSaving(true)
    const { error } = await addPanel({
      projectId: libProjectId,
      name: libPanel.trim(),
      heightIn: parseDim(libH),
      widthIn: parseDim(libW),
    })
    setSaving(false)
    if (error) {
      setToast('Error: ' + error)
    ...
    setLibW('')
    if (libDueDate) {
      await updateDueDate(libProjectId, libDueDate)
    }
  ...
  function handleRemovePanel(panelId: string, panelName: string, projId: string) {
    ...
    const doRemove = async () => {
      const { error } = await removePanel(panelId, projId)
      if (error) setToast('Error: ' + error)
      else setToast(`"${panelName}" removed`)
    }
    if (lc > 0) {
      setWarn({
        title: 'Remove panel?',
        body: `"${panelName}" has ${lc} log ${lc === 1 ? 'entry' : 'entries'}. They'll stay in the log.`,
        ok: 'Remove',
        cancel: 'Cancel',
        danger: true,
        onOk: doRemove,
  ...
  function handleArchive(projId: string, projName: string) {
    ...
    if (hasActive) {
      setWarn({ title: 'Active job running', body: 'Clock out of this project first.', ok: 'OK' })
      return
    }
    setWarn({
      title: 'Archive project?',
      body: `"${projName}" will be hidden. Log entries preserved.`,
      ok: 'Archive',
      cancel: 'Cancel',
      danger: true,
      onOk: async () => {
        const { error } = await archiveProject(projId)
        if (error) setToast('Error: ' + error)
        else setToast(`"${projName}" archived`)
      },
    })
  }
  ...
  function handleTypeChange(projId: string, projName: string, currentType: ProjectType) {
    ...
    setWarn({
      ...
      body,
      ok: `Change to ${toLabel}`,
      cancel: 'Cancel',
      danger: true,
      onOk: async () => {
        const { error } = await updateProjectType(projId, newType)
        if (error) setToast('Error: ' + error)
      },
    })
  }
  ...
  async function saveDue(projId: string) {
    ...
    if (error) setToast('Error: ' + error)
    else setToast('Due date saved')
    setEditingDue(null)
  }

  const sqftPreview =
    libH && libW && parseDim(libH) && parseDim(libW) ? calcSqft(parseDim(libH), parseDim(libW)) : null
  ...
  function renderProject(proj: (typeof sortedProjects)[0], pt: ProjectType) {
    ...
    const donePanelIds = new Set(
      logs.filter(r => r.project_id === proj.id && r.status === 'Complete' && r.panel_id).map(r => r.panel_id!)
    )
    ...
    return (
      <div
        ...
        style={{
          ...
          borderRadius: 16,
          marginBottom: 8,
          border: `1px solid ${pt === 'colorchange' ? CC + '22' : B.border}`,
          overflow: 'hidden',
        }}
      >
        <button
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                {isAdmin && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button
                      onClick={() => handleTypeChange(proj.id, proj.name, proj.project_type)}
                      style={{
                        background: 'none',
                        border: `1px solid ${B.border}`,
                        borderRadius: 8,
                    ...
                    <button
                      onClick={() => handleArchive(proj.id, proj.name)}
                      style={{
                        background: 'none',
                        border: 'none',
                        color: B.red,
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      gap: 4,
                      background: isDone ? B.green + '12' : B.surface2,
                      borderRadius: 20,
                      padding: '6px 12px',
                      border: `1px solid ${isDone ? B.green + '33' : 'transparent'}`,
                    }}
                  >
        ...
        >
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 3, background: B.surface2, borderRadius: 10, padding: 3 }}>
              <button
                onClick={() => setLibProjType('commercial')}
                style={{
                  flex: 1,
                  padding: 9,
                  border: 'none',
                  borderRadius: 8,
                  background: libProjType === 'commercial' ? B.yellow : 'transparent',
                  color: libProjType === 'commercial' ? B.bg : B.textSec,
                  fontWeight: libProjType === 'commercial' ? 700 : 400,
                  fontSize: 13,
                  cursor: 'pointer',
                }}
              >
              ...
              <button
                onClick={() => setLibProjType('colorchange')}
                style={{
                  flex: 1,
                  padding: 9,
                  border: 'none',
                  borderRadius: 8,
                  background: libProjType === 'colorchange' ? CC : 'transparent',
                  color: libProjType === 'colorchange' ? B.text : B.textSec,
                  fontWeight: libProjType === 'colorchange' ? 700 : 400,
                  fontSize: 13,
                  cursor: 'pointer',
                }}
              >
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <select
              value={libProjectId ?? ''}
              onChange={e => setLibProjectId(e.target.value || null)}
              style={{
                padding: '11px 12px',
                borderRadius: 10,
                background: B.surface2,
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <input
                ...
                value={libNewProjectName}
                onChange={e => setLibNewProjectName(e.target.value)}
                style={{
                  flex: 1,
                  padding: '10px 12px',
                  fontSize: 13,
              ...
              <button
                ...
                disabled={saving || !libNewProjectName.trim()}
                style={{
                  padding: '10px 14px',
                  background: libNewProjectName.trim() ? accent : B.surface3,
                  color: libNewProjectName.trim() ? (libProjType === 'colorchange' ? B.text : B.bg) : B.textTer,
                  border: 'none',
                  borderRadius: 10,
                  fontWeight: 700,
                  fontSize: 13,
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <input
              ...
              value={libPanel}
              onChange={e => setLibPanel(e.target.value)}
              onKeyDown={e => {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 4 }}>
            <div>
              ...
              <input
                placeholder="48 or 4ft"
                value={libH}
                onChange={e => {
                  setLibH(e.target.value)
                  setLibHErr('')
                }}
                style={{
                  padding: '10px 12px',
                  ...
                  background: B.surface2,
                  color: B.text,
                  border: libHErr ? `1px solid ${B.red}` : 'none',
                  outline: 'none',
                  width: '100%',
                }}
              />
            ...
            <div>
              ...
              <input
                placeholder="72 or 6ft"
                value={libW}
                onChange={e => {
                  setLibW(e.target.value)
                  setLibWErr('')
                }}
                style={{
                  padding: '10px 12px',
                  ...
                  background: B.surface2,
                  color: B.text,
                  border: libWErr ? `1px solid ${B.red}` : 'none',
                  outline: 'none',
                  width: '100%',
                }}
              />
          ...
          {sqftPreview && (
            <div style={{ fontSize: 12, color: accent, fontWeight: 700, marginBottom: 10, marginTop: 6 }}>
              {sqftPreview.toFixed(2)} sqft preview
            </div>
          )}

          <button
            onClick={handleAddPanel}
            disabled={saving || !libProjectId || !libPanel.trim()}
            style={{
              background: libProjectId && libPanel.trim() ? accent : B.surface3,
              color: libProjectId && libPanel.trim() ? (libProjType === 'colorchange' ? B.text : B.bg) : B.textTer,
              border: 'none',
              borderRadius: 12,
              padding: 12,
              fontWeight: 800,
          ...
          {showBulk && (
            ...
            >
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        onChange={e =>
                          setBulkRows(prev => prev.map((r, i) => (i === idx ? { ...r, name: e.target.value } : r)))
                        }
                        onPaste={e => handleBulkPaste(e, idx, 'name')}
                        style={{
                          padding: '8px 10px',
                          fontSize: 13,
                          borderRadius: 8,
                          background: B.surface3,
                          color: B.text,
                          border: 'none',
                          outline: 'none',
                        }}
                      ...
                      <input
                        ...
                        placeholder="48"
                        onChange={e =>
                          setBulkRows(prev => prev.map((r, i) => (i === idx ? { ...r, h: e.target.value } : r)))
                        }
                        onPaste={e => handleBulkPaste(e, idx, 'h')}
                        style={{
                          padding: '8px 6px',
                          fontSize: 12,
                          borderRadius: 8,
                      ...
                      <div style={{ position: 'relative' }}>
                        <input
                          ...
                          placeholder="72"
                          onChange={e =>
                            setBulkRows(prev => prev.map((r, i) => (i === idx ? { ...r, w: e.target.value } : r)))
                          }
                          onPaste={e => handleBulkPaste(e, idx, 'w')}
                          style={{
                            padding: '8px 6px',
                            fontSize: 12,
                            borderRadius: 8,
              ...
              <button
                ...
                disabled={saving || !libProjectId || !bulkRows.some(r => r.name.trim())}
                style={{
                  background: libProjectId && bulkRows.some(r => r.name.trim()) ? accent : B.surface3,
                  color:
                    libProjectId && bulkRows.some(r => r.name.trim())
                      ? libProjType === 'colorchange'
                        ? B.text
                        : B.bg
                      : B.textTer,
                  border: 'none',
        ...
        return (
          <div key={pt}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {!activeProjs.length && (
                <div style={{ fontSize: 13, color: B.textTer, padding: '8px 2px', marginBottom: 8 }}>No active projects.</div>
              )}
