...
function ProfileDetail({ installerId, onBack }: { installerId: string; onBack: () => void }) {
  ...
  const topInstallerId = useMemo(() => {
    const sqftById = new Map<string, number>()
    for (const r of completeLogs.filter(l => !l.is_color_change)) {
      if (!r.installer_id) continue
      sqftById.set(r.installer_id, (sqftById.get(r.installer_id) ?? 0) + (r.sqft ?? 0))
    }
    let topId = '', topSqft = -1
    sqftById.forEach((v, id) => { if (v > topSqft) { topSqft = v; topId = id } })
    return topId
  }, [completeLogs])

  const myComLogs = useMemo(() =>
    completeLogs.filter(r => r.installer_id === installerId && !r.is_color_change),
    [completeLogs, installerId]
  )
  ...
  // All hooks are above this point — safe to guard after
  const installer = installers.find(i => i.id === installerId) ?? null
  if (!installer) {
    return <div style={{ padding: 32, textAlign: 'center', color: B.textTer, fontSize: 13 }}>Installer not found.</div>
  }
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: B.textSec, fontSize: 13, padding: '0 0 16px 0', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>← All installers</button>
...
export default function Profiles() {
  ...
  if (profileId) {
    return <ProfileDetail installerId={profileId} onBack={() => setProfileId(null)} />
  }
  return <ProfilesList onSelect={setProfileId} />
}
