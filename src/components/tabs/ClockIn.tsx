...
export default function ClockIn() {
  ...
  // ── CRITICAL: derive active job ENTIRELY from DB-sourced activeJobs.
  // Never rely on selectedPanelId/selectedProjectId to decide whether
  // a session is running. This survives refresh, reconnect, lost state.
  const activeJob = selectedInstallerId
    ? activeJobs.find(j => j.installer_id === selectedInstallerId) ?? null
    : null

  // When installer is selected, if they already have an active job in the DB,
  // clear any stale local selection state so the timer view always renders.
  useEffect(() => {
    if (activeJob) {
  ...
  async function handleClockIn() {
    ...
    // On success: activeJobs realtime event will arrive and activeJob will become non-null,
    // which triggers the useEffect above to clear selectedPanelId/selectedProjectId
    // and the UI switches to the timer view automatically.
  }
  ...
  function handleDiscard() {
    ...
    setWarn({
      ...
      ok: 'Discard', cancel: 'Keep going', danger: true,
      onOk: async () => {
    ...
    <div>
      ...
      {showManual && (
        <ManualEntryModal
          ...
          onSave={() => { setShowManual(false); setToast('Manual entry saved') }}
        />
      )}
      ...
      {/* ── Installer selector — always visible regardless of active job ── */}
      ...
      {/* ── Branch: active job exists in DB → always show timer ── */}
      {activeJob ? (
        <div>
        ...
        /* ── No active job → show clock-in form ── */
        <div>
          <div style={{ marginBottom:20 }}>
            ...
            {selectedProject ? (
              <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'13px 16px',borderRadius:12,background: isCC ? CC+'14' : B.yellow+'14',border:`1.5px solid ${accent}` }}>
                ...
                <button onClick={() => { setSelectedProjectId(null); setSelectedPanelId(null); setProjectSearch('') }}
                  style={{ background:'none',border:'none',color:B.textTer,fontSize:20,cursor:'pointer' }}>×</button>
              </div>
            ) : (
              <div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:4,maxHeight:260,overflowY:'auto' }}>
                  {commercial.length > 0 && (
                    <>
                      ...
                      {commercial.map(p => (
                        <button key={p.id}
                          onClick={() => { setSelectedProjectId(p.id); setSelectedPanelId(null); setProjectSearch('') }}
            ...
            <div style={{ marginBottom:20 }}>
              ...
              {(selectedProject.panels ?? []).length > 0 ? (
                <div style={{ display:'flex',flexDirection:'column',gap:4 }}>
                  ...
                  {(selectedProject.panels ?? []).every(pnl =>
            ...
            <div style={{ background:B.surface,borderRadius:14,padding:'14px 16px',marginBottom:20,border:`1px solid ${isCC ? CC+'55' : B.border}` }}>
              <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                <div>
                  ...
                  <div style={{ fontSize:13,color:B.textSec,marginTop:2 }}>{selectedProject.name} · {jobType}</div>
                </div>
                <div style={{ fontSize:12,color:B.textTer }}>
                  {selectedPanelObj.height_in && selectedPanelObj.width_in
                    ? <span style={{ color:B.yellow,fontWeight:700 }}>{calcSqft(selectedPanelObj.height_in, selectedPanelObj.width_in)?.toFixed(2)} sqft</span>
                    : 'dims not set'}
                </div>
              </div>
            </div>
          ...
          <button onClick={handleClockIn}
            ...
            {!selectedInstallerId||!selectedProjectId||!selectedPanelId
              ? 'Select installer, project & panel'
              : `Clock In${isCC ? ' — Color Change' : ''}`}
          </button>
