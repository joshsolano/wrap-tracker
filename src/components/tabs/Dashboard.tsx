...
import { B, fmtTime, weekKey } from '../../lib/utils'

export default function Dashboard() {
  const { logs, installers } = useAppData()

  const completeLogs = useMemo(() => logs.filter(r => r.status === 'Complete' && r.sqft && r.sqft > 0 && r.mins && r.mins > 0), [logs])
  const commercial = useMemo(() => completeLogs.filter(r => !r.is_color_change), [completeLogs])

  const todayStr = new Date().toDateString()
  const todayLogs = useMemo(() => completeLogs.filter(r => r.finish_ts && new Date(r.finish_ts).toDateString() === todayStr), [completeLogs])
  const weekLogs = useMemo(() => commercial.filter(r => weekKey(r.start_ts) === weekKey(new Date().toISOString())), [commercial])
  ...
  function StatCard({ label, value, color, sub }: { label: string; value: string | number; color?: string; sub?: string }) {
    return (
      <div style={{ background:B.surface,borderRadius:14,padding:16,border:`1px solid ${B.border}` }}>
        <div style={{ fontSize:11,color:B.textTer,fontWeight:600,textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:6 }}>{label}</div>
  ...
  function Label({ text }: { text: string }) {
    return <div style={{ fontSize:11,fontWeight:600,color:B.textTer,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:12 }}>{text}</div>
  }
        ...
        {byInstaller.map(r => (
          <div key={r.name} style={{ marginBottom:14 }}>
            <div style={{ display:'flex',justifyContent:'space-between',marginBottom:5,fontSize:13 }}>
              ...
              <span style={{ color:B.textSec }}>{((r.sqft/totalSqftAll)*100).toFixed(1)}% · {r.sqft.toFixed(1)} sqft</span>
            </div>
            <div style={{ height:5,background:B.surface2,borderRadius:3,overflow:'hidden' }}>
              <div style={{ height:'100%',width:`${(r.sqft/totalSqftAll)*100}%`,background:r.color,borderRadius:3 }} />
            </div>
          </div>
        ))}
